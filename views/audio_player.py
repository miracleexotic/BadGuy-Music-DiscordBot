...
class SongReqModal(discord.ui.Modal):
    ...
    def __init__(self, *, title: str = 'Music Request', bot: commands.Bot, ctx: commands.Context, view: discord.ui.View) -> None:
    ...
    async def on_submit(self, interaction: discord.Interaction):
        async with self.ctx.typing():
            try:
                source = await ytdl.YTDLSource.create_source(self.ctx, self.name.value, loop=self.bot.loop)
            except ytdl.YTDLError as e:
                await self.ctx.send('An error occurred while processing this request: {}'.format(str(e)))
            else:
...
class AudioPlayerView(discord.ui.View):
    ...
    async def songreq(self, interaction: discord.Interaction, button: discord.ui.Button):
